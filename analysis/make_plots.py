...
test_result/final_result/.
  ...
  1. combo_tau08 / combo_tau09  - per-task states (bars, left axis) plus
     training & validation agreement (markers, right axis), one figure per
     threshold.
  2. baseline_comparison        - 3-panel groupplot: final states, final
     validation agreement, and runtime, BeamSearch vs SA vs GA vs PSO.

Each .tex is a full \\begin{figure}...\\end{figure} block - \\input{} it
directly into the thesis body. Preamble requirements:
    \\usepackage{pgfplots}
    \\pgfplotsset{compat=1.18}
    \\usepgfplotslibrary{groupplots}   (baseline_comparison only)
...
# panels (a)/(b)/(c) of baseline_comparison line up in the same columns.
...
def combo_figure(rows, threshold: float, batch_size: int = 1000) -> str:
    ...
    train_coords = " ".join(f"({SHORT_LABEL[t]},{recs[t]['train_final']:.2f})" for t in TASKS)
    val_coords = " ".join(f"({SHORT_LABEL[t]},{recs[t]['val_final']:.2f})" for t in TASKS)

    max_states = max(recs[t]["initial_states"] for t in TASKS)
    y_max_states = (max_states // 5 + 2) * 5
    min_agreement = min(min(recs[t]["train_final"], recs[t]["val_final"]) for t in TASKS)
    y_min_agreement = min(0.6, round(min_agreement - 0.05, 2))
...
\\addlegendentry{{Training agreement}}
\\addlegendimage{{only marks, color=valColor, mark=square*, mark size=2.4pt}}
\\addlegendentry{{Validation agreement}}
\\end{{axis}}

% ===== Right axis: training / validation agreement =====
\\begin{{axis}}[
    width=13cm, height=7cm,
    axis y line*=right,
    axis x line=none,
...
\\addplot[color=gray!65, dashed, line width=0.8pt]
    coordinates {{({SHORT_LABEL[TASKS[0]]},{threshold}) ({SHORT_LABEL[TASKS[-1]]},{threshold})}};
\\addplot[only marks, color=trainColor, mark=diamond*, mark size=2.8pt,
    nodes near coords, every node near coord/.append style={{font=\\footnotesize,anchor=south,yshift=2pt}}]
    coordinates {{{train_coords}}};
\\addplot[only marks, color=valColor, mark=square*, mark size=2.4pt,
    nodes near coords, every node near coord/.append style={{font=\\footnotesize,anchor=north,yshift=-2pt}}]
    coordinates {{{val_coords}}};
\\end{{axis}}
\\end{{tikzpicture}}
\\caption{{Number of DFA states before and after refinement (bars) and final training/validation agreement (markers) across the six tasks under agreement threshold $\\tau={threshold}$. The dashed line indicates the specified agreement threshold.}}
...
# ---------------------------------------------------------------------------
# Figure 3 (Table 5.5): BeamSearch vs. SA/GA/PSO - states, validation
# agreement, and runtime, as a 3-panel groupplot.
# ---------------------------------------------------------------------------
def baseline_comparison_figure(rows, threshold: float = 0.8, batch_size: int = 1000) -> str:
    ...
    if missing:
        raise ValueError(f"baseline_comparison_figure(tau={threshold}): missing rows for {missing}")
    ...
    train_agreement = series("train_final", lambda v: f"{v:.3f}")
    val_agreement = series("val_final", lambda v: f"{v:.3f}")
    runtime = series("time_s", lambda v: f"{v:.1f}")

    all_states = [by_method[t][m]["final_states"] for t in TASKS for m in METHODS]
    all_agreement = ([by_method[t][m]["train_final"] for t in TASKS for m in METHODS]
                      + [by_method[t][m]["val_final"] for t in TASKS for m in METHODS])
...
\\end{{groupplot}}
\\end{{tikzpicture}}
\\caption{{Comparison between the proposed method (Beam) and baseline search algorithms (SA, GA, and PSO) across six tasks under batch size {batch_size} and $\\tau={threshold}$: (a) final DFA size, (b) training agreement, (c) validation agreement, and (d) runtime on a logarithmic scale.}}
...
\usepackage[margin=1cm,paperwidth=17cm,paperheight=26cm]{geometry}
\usepackage{pgfplots}
\pgfplotsset{compat=1.18}
...
def main():
    ...
    figures = {
        ...
        "baseline_comparison": baseline_comparison_figure(rows, threshold=0.8),
    }
